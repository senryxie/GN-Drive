...
class APIClient(object):
    ...
    def get_authorize_url(self, oauth_token, oauth_callback='json'):
        '''
        ...
        '''
        return '%s/oauth/authorize?oauth_token=%s&oauth_callback=%s' % (self.api_url, oauth_token, oauth_callback)
...
if __name__=='__main__':
    ...
    print '申请request token'
    oauth_token = client.get_request_token()


    print '鉴权（有多种方式，这里用用户名密码）'
    url = client.get_authorize_url(oauth_token.oauth_token)
    url += '&userId=%s&passwd=%s' % ('ofshellohicy@126.com', 'douban6886033')
    ret = urllib2.urlopen(url)
    #返回的认证token
    data = json.load(ret)

    client.oauth_token = data[u'oauth_token'].encode('utf-8')
    client.oauth_verifier = data[u'oauth_verifier'].encode('utf-8')

    print '获取access token'
    access_token = client.get_access_token()
    print access_token

    print '使用access token'
    client.oauth_token = access_token.oauth_token
    client.oauth_token_secret = access_token.oauth_token_secret
    print 'access token: ', client.oauth_token
    print 'token secret: ', client.oauth_token_secret

    #client.post.statuses__update(status="来一发")
